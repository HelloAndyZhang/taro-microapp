...
export default (ctx: IPluginContext) => {
  ...
  ctx.modifyRunnerOpts(({ opts }) => {
    ...
    if (compiler.type === 'webpack5') {
      ...
      prebundleOptions.exclude.push(
        '@tarojs/plugin-platform-alipay/dist/runtime',
      );
...
function modifyPageTemplate(ctx: IPluginContext) {
  ...
  ctx.modifyBuildAssets(({ assets, miniPlugin }) => {
    ...
    miniPlugin.pages.forEach((page) => {
      const config =
        miniPlugin.filesConfig[miniPlugin.getConfigFilePath(page.name)].content;
      if (
        !page.isNative &&
        config?.hasOwnProperty('usingComponents') &&
        Object.keys(config.usingComponents).length
      ) {
    ...
    const baseXml =
      assets[
        getIsBuildPluginPath('base.axml', miniPlugin.options.isBuildPlugin)
      ].source();

    pages.forEach((page) => {
      const templateName = `${page}.axml`;
      const assetsItem = assets[templateName];
      const src = assetsItem._value
        ? assetsItem._value.toString()
        : assetsItem.source();
      let relativePath;
      const templateCaller = src.replace(
